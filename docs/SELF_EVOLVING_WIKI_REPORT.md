...
- 정식 URL·DOI·저장소 신원, 상태, 의존성 묶음, 반증 검색을 포함한 출처 입수
- admission allow를 우회할 수 없는 canonical `source-add` writer gate
- 결정론적 BM25 계열 검색과 의존성·의미 충돌 후보 미리보기
- 보정·보안·실행 시간 고정 자료와 해당 자료의 해시 고정
- 구조·OKF·fixture·receipt·실제 test를 결합하는 `release-check`
- 입수 판정과 외부 보고서 다이제스트의 사건 사슬 기준점, 추가 전용 상태·OKF 투영, 되돌리기 예행연습
...
```text
불변 원문 → 색인 → LLM이 관리하는 Markdown 위키
```
...
- 벤치마크 게이트를 거치는 하네스 자기 발전

따라서 초기 시스템은 다섯 층으로 확장했다.

```text
1. 출처 증거          불변 스냅샷, 해시, URL, 라이선스, 게시 상태
2. 인식론적 원장      원자적 주장, 정확한 위치, 지지·반박, 시점
3. Living Wiki         개념, 비교, 종합, 관점, 질문
4. 협업                행위자, 기여, 검토, 결정, 캠페인
5. 제어와 발전        정책, 평가, 보안 게이트, RFC, 롤백, 감사 사건
```
...
- 프롬프트 주입, 메모리 오염, 개인정보 보호, 저작권, 동시 편집, 릴리스 평가가 없다.
...
| 구간 | 내용 | 설계 의미 |
|---|---|---|
| 00:00–09:16 | 10,994개 수준의 흩어진 노트를 다음 연구에서 재사용하지 못함 | 문제는 맥락 양보다 지속 기억과 라우팅 |
| 09:16–12:48 | 로컬 Markdown, Obsidian, 여러 연결기 | 검사 가능하고 로컬 우선인 원문 |
| 12:48–15:29 | V1: 선별 링크 + 심층 조사 → 정적 `research.md` | 좋은 씨앗과 여러 검색 회차 |
| 15:29–18:28 | V2: 개인 Second Brain까지 검색, 여전히 정적 결과 | 개인화되지만 새 질문마다 전체 재실행 |
| 18:28–25:01 | V3: 원문 + `index.yaml` + 가변 Wiki | 점진적 맥락 적재와 지속 합성 |
| 25:01–27:04 | 전체 보관소의 읽기 전용 스냅샷과 프로젝트 Wiki 분리 | 전역 메모리와 작업 메모리 분리 |
| 27:04–36:50 | 웹·GitHub·사용자 링크 시연 | 실제 Agent 중심 작업 절차 |
| 36:50–38:45 | 출처 추적성, 출처 강도·순위, 린트, 압축 미완성 인정 | 사용자의 신뢰 요구가 정확히 남은 문제 |
...
- `contradictions.md`, 미해결 질문, 질문 페이지
- 질의·추가·심층·초기화 경로 선택
- 빠름·가벼움·심층 조사 깊이
- YouTube·GitHub·웹·PDF·Obsidian·Readwise·NotebookLM 연결기
- 결정론적 색인 생성
- 고립 문서, 누락된 중심·비교 문서, 끊어진 링크를 찾는 린트
...
```text
저장소 제어 영역
├── state/                   행위자, 출처, 주장, 검토, 캠페인, 사건
├── raw/                     불변 증거
├── governance/              거버넌스
├── evaluations/             평가
├── tools/                   도구
└── wiki/                    ← 정확한 OKF 번들 경계
    ├── index.md             ← 예약 파일, 앞부분 메타데이터 없음
    ├── log.md               ← 예약 파일, 앞부분 메타데이터 없음
    ├── okf-profile.md       ← v0.1 프로필을 선언하는 type: Concept 문서
    ├── sources/*.md         ← type: Reference인 출처 문서
    ├── concepts/*.md        ← type: Concept인 개념 문서
    └── perspectives/*.md    ← type: Position인 관점 문서
```
...
| S2 | 출처 이력은 있으나 실무자 자체 보고, 단일 사례, 사전 공개본 등 추가 검증 필요 | 귀속된 증거 |
...
- 원제: [From Untrusted Input to Trusted Memory](https://arxiv.org/abs/2606.04329) — 쓰기 채널 네 개, 구조 취약점 아홉 개, 공격 유형 여섯 개, `MPBench`
- 원제: [SafeSearch](https://openreview.net/forum?id=95VVL0TJNH) — 검색 Agent의 허위정보·프롬프트 주입 적대적 평가
- 원제: [MM-PoisonRAG](https://aclanthology.org/2026.acl-long.1558/) — 다중양식 지식 오염
- 원제: [OWASP Agent Memory Guard](https://owasp.org/www-project-agent-memory-guard/) — 해시·스냅샷·정책·롤백 중심 참고 구현

초기 보안 규칙:

- 수집 Agent/파서와 canonical memory writer의 권한 분리
- 외부 콘텐츠의 “이 지시를 따르라”는 문장을 실행하지 않음
- 파일 형식·크기·해시·출처·라이선스 기록
...
- 원문 → 색인 → Markdown Wiki
- Karpathy·AI Research OS의 영속 종합과 점진적 공개
...
- 예약된 루트·하위 디렉터리 색인과 갱신 로그
- Living Wiki 확장 매핑과 OKF 검증기
- trust/governance 원장은 바깥 control plane에 유지

#### v4 — 폐쇄 순환형 하네스

- 행위자 중립 협업의 방향·교정·단서·이의
- 실행 주기 기반 관심사 생성과 계획 전용 범위 제한 실행 환경
- 출처 신원·상태·반증 검색 입수와 격리 보안 게이트
...
- 유한한 v3.1 기존 출처 예외 manifest, 마이그레이션, 롤백 예행연습
- 남은 실패: 장기 실증 보정, 미관측 의미·다중양식 적대적 검증, 실제 운영 어댑터·실행기, 다중 작성자 서명·잠금
...
│   └── trust-policy.json        S0-S4·C0-C4 게이트
├── state/                       정식 상태 원장
│   ├── actors.json              행위자 원장
│   ├── sources.json             출처 원장
│   ├── claims.json              주장 원장
│   ├── reviews.json             검토 원장
│   ├── campaigns.json           캠페인 원장
│   ├── proposals.json           제안 원장
│   ├── collaborations.json      협업 원장
│   ├── admissions.json          입수 판정 원장
│   ├── runs.json                실행 원장
│   └── events.jsonl             해시 사슬 추가 전용 사건 로그
├── raw/                         불변 출처와 격리 자료 저장소
├── wiki/                        이식 가능한 OKF v0.1 파생 지식 번들
├── research/                    캠페인과 연구 기록
├── governance/                  헌장, ADR, RFC
├── evolution/                   v1→v2→v3.1→v4 실패·마이그레이션 이력
├── evaluations/                 고정 자료, 영수증, 릴리스 보고서, 스냅샷
├── migrations/                  고정된 기존 항목 예외·마이그레이션 명세
├── reports/                     린트와 보고서
├── prompts/research-cycle.md    연구 순환 프롬프트
├── scripts/research-cycle.sh    연구 순환 스크립트
├── tools/wiki.py                표준 라이브러리 전용 CLI
└── tests/test_wiki.py           단위 테스트
```
...
- 보정 예비 평가 15건, 보안 고정 말뭉치 31건, 고정 실행 시간 시나리오
...
- 원제: [LiveResearchBench](https://openreview.net/forum?id=ghwbZ3uhEd), `ICLR 2026 Poster` — 동적·실시간 연구, 인용, 사용자 정렬 벤치마크. 실제 웹 재현성 문제가 있다.
...
- 원제: [Data-Centric Perspectives on Agentic RAG](https://aclanthology.org/2026.findings-acl.78/), `Findings ACL 2026` — 데이터 수명주기 조사. 아키텍처 우월성의 증명은 아니다.
...
- 원제: [Hindsight](https://aclanthology.org/2026.acl-demo.27/), `ACL 2026 Demo` — 세계·경험·관찰·의견 네트워크 분리. 관점 계층의 직접 참고 자료다.
- 원제: [MAGMA](https://aclanthology.org/2026.acl-long.1709/), `ACL 2026` — 의미·시간·인과·개체 다중 그래프. 그래프 추출 오류는 별도 문제다.
- 원제: [Chronos](https://aclanthology.org/2026.findings-acl.546/), Findings ACL 2026 — 지속되는 지식 변화와 사건 발전 그래프.
- 원제: [LEDGER](https://aclanthology.org/2026.findings-acl.515/), Findings ACL 2026 — 의존성을 고려한 범위 제한 문서 편집.
- 원제: [Dissecting GraphRAG](https://aclanthology.org/2026.tacl-1.29/), `TACL 2026` — 삼중항·커뮤니티 병목과 결정론적 템플릿의 가치.
- 원제: [SkillWiki](https://arxiv.org/abs/2606.16523), 2026년 사전 공개본 — 살아 있는 Agent Skill 지식 수명주기. 사실형 Wiki가 아닌 Skill 인프라라는 범위 한계가 있다.
...
- 원제: [GenProve](https://aclanthology.org/2026.acl-long.228/), ACL 2026 — 인용·압축·추론의 출처 이력.
- [FRANQ](https://aclanthology.org/2026.findings-acl.338/), Findings ACL 2026: faithfulness와 factuality 분리.
- 원제: [Assessing Web Search Credibility and Response Groundedness](https://aclanthology.org/2026.eacl-long.115/), EACL 2026 — 전달자와 메시지 근거성을 함께 평가.
- 원제: [Uncertainty Quantification in LLM Agents](https://aclanthology.org/2026.acl-long.738/), ACL 2026 — 경로·개체별 불확실성의 필요성.
...
- 원제: [Adaptive Collaboration with Humans](https://openreview.net/forum?id=IKVUB9Exuc), ICLR 2026 — 비용·위험을 고려한 보류 정책.
- 원제: [InterDeepResearch](https://arxiv.org/abs/2603.12608), 2026 사전 공개 논문 — 관찰 가능성, 조향 가능성, 맥락 탐색.
- 원제: [Evolving Agentic Workflow Driven by Human-Agent Collaboration](https://aclanthology.org/2026.findings-acl.1250/), Findings ACL 2026 — 사람 선호에 따른 작업 흐름 발전이며 사실 거버넌스는 별도다.
- [AgentBound](https://arxiv.org/abs/2606.30970), 2026년 사전 공개본: 검증 가능한 행동 거버넌스와 행동 영수증.
- 원제: [Regulating the Machine Contributor](https://arxiv.org/abs/2606.14594), 2026 사전 공개 논문 — 공개, 책임, 감독, 라이선스, 집행.

### 보안과 오염

- 원제: [A-MemGuard](https://openreview.net/forum?id=udqe7UZUZ6), ICML 2026 — 메모리 오염 방어.
- 원제: [From Untrusted Input to Trusted Memory](https://arxiv.org/abs/2606.04329), 2026 사전 공개 논문 — 메모리 쓰기 경로, 취약점 분류, MPBench.
...
- 원제: [MM-PoisonRAG](https://aclanthology.org/2026.acl-long.1558/), ACL 2026 — 다중양식 지식 오염.
- 원제: [SafeSearch](https://openreview.net/forum?id=95VVL0TJNH), ICML 2026 — 검색 Agent 적대적 평가.
...
- [W3C PROV-O](https://www.w3.org/TR/prov-o/): 개체·활동·Agent와 파생·귀속. 출처 이력이지 진실 인증서가 아니다.
- [Nanopublications](https://nanopub.net/): assertion/provenance/publication-info 분리.
- 원제: [Wikidata data model](https://www.wikidata.org/wiki/Help:Data_model) — 문장 한정자·참조·순위. 순위는 신뢰도가 아니다.
- 원제: [NIST AI RMF](https://www.nist.gov/itl/ai-risk-management-framework) — 거버넌스·지도화·측정·관리 릴리스 점검표.
...
- 원제: [SAFE](https://deepmind.google/research/publications/85420/) — 검색 증강 장문 사실성 평가.

## 15. AI Engineer 채널 후속 관찰 목록

영상은 구현 패턴과 실패 사례를 얻는 practitioner source로 좋지만, 논문과 같은 신뢰 등급을 자동 부여하지 않는다.

- 영상 원제: [Build Your Own Deep Research Agents](https://www.youtube.com/watch?v=mYSRn6PC1mc), 2026-04-20 — 웹·YouTube·GitHub 검색, 신뢰 필터링, 인용 자료, 평가 실습.
...
- 영상 원제: [Continual Learning for AI Agents](https://www.youtube.com/watch?v=2IxD9OB3XuQ), 2026-07-05 — 실패 재생과 회귀를 고려한 순환.
...
- 영상 원제: [Production Evals for Agentic AI Systems](https://www.youtube.com/watch?v=vljxQZfJ9wY), 2026-06-25 — 지속 운영 평가와 원격 측정.
- 영상 원제: [Capturing Decisions for Humans and AI Alike](https://www.youtube.com/watch?v=504PvfXou5Y), 2026-06-03 — `ADR`·`BDD`·`Git`·`CI`로 정책을 실행 가능하게 만들기.
