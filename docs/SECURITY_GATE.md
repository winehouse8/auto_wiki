...
4. 검사기에는 네트워크, 하위 프로세스·셸, 동적 가져오기, 자격증명 API 기능이 없다.
...
- `signals`: 규칙 ID, 범주, 위험도, 설명, 범위, 짧게 가린 발췌, 맥락 보정값
...
| 범주 | 예시 신호 | 해석 |
|---|---|---|
| 프롬프트 주입 | 이전 지시 무시, 역할 재할당, 가짜 시스템 블록, 숨은 지시 | 출처 데이터가 지침 우선순위를 탈취하려 함 |
| 비밀 유출 | `.env`, SSH 개인 키, 특권 프롬프트를 읽거나 전송하라는 요구 | 비밀 또는 상위 지침 유출 시도 |
| 셸 명령 | 내려받아 실행, 파괴적 삭제, 실행 권한, 역방향 연결 | 외부 텍스트가 실행 능력을 요구함 |
| 정책 덮어쓰기 | `AGENTS.md`, 헌장, 신뢰 정책 변경 또는 게이트 우회 | 지속 정책과 신뢰 상태 변조 시도 |
| 지속성 | 정규 메모리 저장, 미래 요청 반복 실행, Wiki 지시 삽입 | 한 번의 입력을 장기 명령으로 만들려 함 |
| 난독화 | 긴 base64 유사 blob | 어휘 검사기가 내용을 검사하지 못하게 함 |
| 콘텐츠 무결성 | 미디어 불일치, 잘못된 UTF-8, 과대 크기, 불투명 바이너리 | 자동 판정에 필요한 관찰이 불완전함 |
| 실행 payload | ELF·PE 서명 | 콘텐츠 자체가 실행 원문임 |
...
- 공격 18건: 프롬프트 재정의, 역할 재할당, 비밀·SSH 유출, 다운로드 후 실행, 파괴적 셸, 정책 덮어쓰기, 신뢰 강제, 지속성, 숨은 HTML, 난독화, PDF 샌드박스 추출, YouTube 자막, 저장소 README
...
|---|---:|
| 공격 사례 탐지율 | 1.0 |
| 전체 단계 공격 성공률 (`allow` 비율) | 0.0 |
| 쓰기·검색·활성화 공격 성공률 | 각각 0.0 |
| 정상 사례 강제 거부율 | 0.0 |
| 정상 사례 검토율 | 0.384615 |
...
`tests/test_security_gate.py`는 명세 목록·해시, 매체 불일치, 원문·정규화 분리, 프롬프트·비밀정보·셸·정책·지속성 탐지, 쓰기·검색·활성화 판정, 이진 파일·실행 파일·크기 초과, 오탐, 결정론적 보고서·CLI, 공격 성공률과 정상 자료 거부율, 네트워크·프로세스 권한 부재를 검사한다.
