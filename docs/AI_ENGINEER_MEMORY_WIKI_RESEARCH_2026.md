# AI Engineer 2026 Memory·Wiki 연구 감사 보고서
...
5. 고정한 제목·설명에 문서화한 직접/인접/제외 규칙을 적용했다. 직접 후보 34개 모두의 auto-caption 입수를 시도하고 각각 security receipt와 `promote|defer|exclude` 처분을 남겼다.
...
| 분류 | 수 | 포함 기준 | 대표 예시 | 처리 |
|---|---:|---|---|---|
| 직접 관련 | 34 | 장기·지속 memory, knowledge base/Wiki, second brain, context lifecycle·hygiene, failure-driven learning, event-sourced continuity를 주제로 직접 주장하거나 mixed event metadata에 명시적 관련 chapter가 있음 | *Demand-Driven Context*, *The Log Is The Agent*, *Turn 10,994 Notes Into Memory*, *Continual Learning for AI Agents* | 자막 입수 34/34; 보안 allow 31, reject 3; 처분 promote 14, defer 17, exclude 3. 그중 7개 신규 S2 source admission, 1개 기존 S2 재검토 |
| 인접 관련 | 32 | RAG/search, MCP·skill context, long context/cache, evaluation·observability가 핵심이며 지속 Wiki 자체는 부차적 | *How we taught agents to use good retrieval*, *RAG is dead, right??*, *Agentic Search for Context Engineering*, *MCP = Mega Context Problem* | 설계 counterpoint와 후속 조사 lead로만 보존 |
| 제외 | 196 | UI 생성, 음성·영상, 일반 coding demo, 배포, 영업·행사 소개 등 이번 연구 질문에 직접 답하지 않음 | 개별 제목 생략 | evidence로 사용하지 않음 |
| 완료·시청 가능 합계 | 262 | 조사 시점 공개 tab에서 중복 제거되고 기간 필터를 통과한 완료 항목: regular 251, livestream replay 11, Shorts 0 |  | 의미 분류의 분모 |
| 예약 premiere | 2 | 기간 내 목록에는 나타났지만 snapshot 시점에 아직 시청 불가 |  | manifest에는 보존, 연구 분모에서는 제외 |
| 목록 행 합계 | 264 | 완료·시청 가능 262 + 예약 premiere 2 |  |  |
...
| Source ID | 자료·날짜 | 수준·독립성 그룹 | 보존·status | 이 보고서에서의 역할 |
|---|---|---|---|---|
| `SRC-0800355B8885` | AI Engineer channel audit bundle, 2026-07-12 snapshot | S2, `youtube-ai-engineer-channel-audit-2026-07-12` | JSON artifact 보존, SHA-256 `bea5e…3980` | 264개 전수 목록, 262개 분모, 34/32/196 분류, 34개 자막 gate·처분의 coverage 근거; 단일 agent 분류라 C1 한정 |
| `SRC-F55FED177366` | [Demand-Driven Context 영상](https://www.youtube.com/watch?v=_QAVExf_1uw), 2026-05-05 | S2, `ddc-authors` | metadata-only, published video | 방법 발견 lead; 같은 저자 preprint와 독립 아님 |
| `SRC-AD0B1D50C531` | [Continual Learning for AI Agents](https://www.youtube.com/watch?v=2IxD9OB3XuQ), 2026-07-05 | S2, `relai-vcl` | metadata-only, published video | replay와 regression 중심 harness 원칙 lead |
| `SRC-03641BCFC467` | [How Lovable self-improves every hour](https://www.youtube.com/watch?v=KA5kPbdkK2E), 2026-06-02 | S2, `lovable-self-improvement` | metadata-only, published video | production feedback loop 사례 |
| `SRC-2E2EA9C214C1` | [The Log Is The Agent](https://www.youtube.com/watch?v=UPwGaM2MKHY), 2026-06-25 | S2, `omnara-log-agent` | metadata-only, published video | event log와 외부 상태 경계 lead |
| `SRC-3F8E6D0FDE7E` | [How we solved Context Management in Agents](https://www.youtube.com/watch?v=esY99nYXxR4), 2026-05-10 | S2, `arize-alyx-context` | metadata-only, published video | truncation·summary·retrievable spill failure lead |
| `SRC-7409811C56EB` | [Context Is the New Code](https://www.youtube.com/watch?v=bSG9wUYaHWU), 2026-05-03 | S2, `tessl-context-lifecycle` | metadata-only, published video | context lifecycle framing |
| `SRC-9BADA4274C74` | [User Signal Dies at the Retrieval Boundary](https://www.youtube.com/watch?v=Jx4ZFEAq6bY), 2026-06-28 | S2, `starlight-utility-memory` | metadata-only, published video | outcome feedback 문제 lead; utility auto-ranking은 거부 |
| `SRC-1C96ABEBBA41` | [Turn 10,994 Notes Into Memory](https://www.youtube.com/watch?v=ZRM_TfEZcIo), 2026-06-26 | S2, `ai-research-os-authors` | conference practitioner talk, immutable snapshot 없음 | raw→index→Wiki와 persistent synthesis의 기존 seed |
| `SRC-54D07435EB56` | [Demand-Driven Context preprint](https://arxiv.org/abs/2603.14057), 2026-03-14 | S2, `ddc-authors` | preprint, metadata-only | 영상의 원 방법·한계 확인; peer review 아님 |
| `SRC-AF06BCDC1ED2` | [How Memory Management Impacts LLM Agents](https://aclanthology.org/2026.acl-long.27/), ACL 2026 | S3, `xiong-memory-management` | peer-reviewed; CC BY 4.0 추출 text 보존 | 오류 memory 재사용·downstream feedback의 독립 실증 근거 |
| `SRC-9639A8245BE8` | [Agentic Memory](https://aclanthology.org/2026.acl-long.981/), ACL 2026 | S3, `agemem-authors` | peer-reviewed; artifact는 gate에서 미승격 | 명시적 memory lifecycle action의 연구 맥락 |
| `SRC-F9BA839FA59D` | [Controllable Memory Usage](https://aclanthology.org/2026.acl-long.670/), ACL 2026 | S3, `steem-memory-control` | peer-reviewed; CC BY 4.0 추출 text 보존 | anchoring과 사용자 memory 의존도 제어 근거 |
...
## 3. Deep shortlist 비판적 분석

### 3.1 Demand-Driven Context — failure를 지식 공백 신호로 사용

- 영상: [Demand-Driven Context: A Methodology for Coherent Knowledge Bases Through Agent Failure](https://www.youtube.com/watch?v=_QAVExf_1uw), 2026-05-05
...
### 3.2 Continual Learning for AI Agents — replay 가능한 실패에서 최소 변경

- 영상: [Continual Learning for AI Agents: From Failures to Durable Improvements](https://www.youtube.com/watch?v=2IxD9OB3XuQ), 2026-07-05
- 발표자: Soheil Feizi, University of Maryland 교수이자 RELAI founder/CSO
...
### 3.3 Lovable — production friction을 PR 후보로 바꾸는 인간 승인 loop

- 영상: [How Lovable self-improves every hour](https://www.youtube.com/watch?v=KA5kPbdkK2E), 2026-06-02
...
### 3.4 The Log Is The Agent — event log와 외부 상태의 경계

- 영상: [The Log Is The Agent](https://www.youtube.com/watch?v=UPwGaM2MKHY), 2026-06-25
- 발표자: Ishaan Sehgal, Omnara CEO
- 현재 제품: [Omnara](https://www.omnara.com/)
- 글 mirror: [The Log Is The Agent](https://dev.to/dailycontext/the-log-is-the-agent-5096)
...
- model/tool/prompt/config version, workspace와 uncommitted file, credential state, network response, 외부 서비스 side effect를 어디까지 snapshot하는가.
...
### 3.5 Arize — working context hygiene와 retrievable spill

- 영상: [How we solved Context Management in Agents](https://www.youtube.com/watch?v=esY99nYXxR4), 2026-05-10
- 발표자: Sally-Ann DeLucia, Arize
- 공식 글: [How to manage LLM context windows for AI agents](https://arize.com/blog/how-to-manage-llm-context-windows-for-ai-agents/), [Context management in agent harnesses](https://arize.com/blog/context-management-in-agent-harnesses/)
...
- long-session evaluation의 task, model, sample size, raw comparison, cost.
...
### 3.6 Tessl CDLC — context를 versioned artifact로 관리

- 영상: [Context Is the New Code](https://www.youtube.com/watch?v=bSG9wUYaHWU), 2026-05-03
...
### 3.7 Utility feedback — 결과 신호를 retrieval에 돌려보내되 신뢰와 분리

- 영상: [User Signal Dies at the Retrieval Boundary](https://www.youtube.com/watch?v=Jx4ZFEAq6bY), 2026-06-28
...
### 3.8 AI Research OS — persistent synthesis의 seed이지 완결된 trust system은 아님
...
| 날짜 | 자료 | Messenger/message와 이해상충 | 미채택 이유 |
|---|---|---|---|
...
| 2026-05-16/28/29 | [Connecting the Dots](https://www.youtube.com/watch?v=eW_vxrjvERk), [Decision-Aware Agents](https://www.youtube.com/watch?v=abvQEhvRI_c), [Decision Traces](https://www.youtube.com/watch?v=B9h9ovW5H9U) — Neo4j | graph database vendor가 entity·decision trace memory를 제안; [agent-memory](https://github.com/neo4j-labs/agent-memory)는 공개 | 세 영상·blog·repo가 모두 한 vendor 그룹이며 graph 우월성과 rationale fidelity의 독립 평가가 없음 |
...
| 2026-07-07 | [How we taught agents to use good retrieval — Hanna Lichtenberg, Mixedbread](https://www.youtube.com/watch?v=1IdzkRVmWAA) | retrieval vendor의 bounded multi-query loop | relevance 개선과 messenger/source credibility 개선은 다른 문제이며 독립 benchmark가 없음 |
...
## 5. 원 논문 교차검토와 venue status

### 5.1 DDC preprint는 peer-reviewed corroboration이 아니다
...
### 5.3 memory lifecycle action의 존재와 Wiki 자동화의 효능은 다르다
...
### 5.5 venue와 source level 요약

| 자료 | 공개 상태 | 독립성 | 말할 수 있는 것 | 말할 수 없는 것 |
|---|---|---|---|---|
| DDC preprint | arXiv preprint, S2 | 영상과 같은 저자 그룹 | 방법 정의, worked example, 저자 명시 한계 | 수렴·효능의 독립 재현 |
| ACL 2026 long.27 | main conference peer-reviewed, S3 | DDC/vendor와 독립 | 오류 memory 재사용과 outcome feedback의 실험적 위험 | Wiki 자동 삭제·trust mutation의 정당성 |
| ACL 2026 long.981 | main conference peer-reviewed, S3 | 다른 저자 그룹 | 명시적 memory action을 평가할 수 있음 | learned discard policy의 Wiki 적용 효능 |
| ACL 2026 long.670 | main conference peer-reviewed, S3 | 다른 저자 그룹 | memory anchoring과 사용자 의존도 제어 | factual Wiki의 정확도 개선 |

## 6. 신규 claim과 현재 신뢰 경계

| Claim ID | 현재 상태 | 연결 evidence | 채택한 경계 | 아직 증명되지 않은 것 |
|---|---|---|---|---|
| `CLM-95A38CACF2CD` | C2/supported | `SRC-54D07435EB56` support, `SRC-AF06BCDC1ED2` support, `SRC-F55FED177366` context | failure를 조사 우선순위 신호로 사용하고 독립 evidence와 replay 뒤에만 승격 | failure가 곧 knowledge gap이라는 인과 판단, DDC 수렴·효율 |
| `CLM-F79558D817DF` | C2/supported | `SRC-AF06BCDC1ED2` support; `SRC-03641BCFC467`, `SRC-9639A8245BE8`, `SRC-F9BA839FA59D`, `SRC-9BADA4274C74` context | downstream outcome·시간 상태를 audit하되 trust·삭제와 분리 | utility ranking의 효능, 자동 staleness 판정, 자동 discard |
| `CLM-EC52C0576A28` | C2/supported | `SRC-F9BA839FA59D` support | fresh-start를 삭제가 아닌 독립 재조사 retrieval policy로 구현 | personalized benchmark 결과의 factual Wiki 일반화 |
| `CLM-207429D54323` | C1/supported | `SRC-2E2EA9C214C1` support, PROV-O source context | log는 감사 원장, 외부 상태에는 snapshot/version/digest/receipt 필요 | log alone이 agent identity와 전체 실행 상태를 복원한다는 주장 |
| `CLM-F5982AA50A01` | C1/supported | `SRC-0800355B8885` JSON coverage fields | 목록 264와 완료·시청 가능 분모 262, 예약 2를 구분 | 삭제·비공개·unlisted 영상 또는 미래 metadata 상태 |
| `CLM-0733D26931D2` | C1/supported | `SRC-0800355B8885` rules, per-row classification, metadata hashes | 직접 34·인접 32·제외 196이라는 감사 가능한 단일 agent 선별 | 독립 분류자 간 일치도나 영상 내용의 진실성 |
| `CLM-F63FA5329493` | C1/supported | `SRC-0800355B8885` 34개 security receipt·review disposition | 자막 처리 coverage와 reject 무우회 | 자막 입수가 곧 내용 신뢰성 또는 재배포 권리를 뜻한다는 주장 |
...
1. **Audit-only memory feedback ledger**
...
2. **Read-only deterministic memory hygiene**
...
| 제안 | 결정 | 이유 |
|---|---|---|
| YouTube/video claim을 사실 claim로 직접 승격 | 거부 | S2 practitioner/vendor lead이며 방법·raw result·독립 재현 부족 |
| 같은 회사의 영상·blog·docs·repo를 여러 독립 근거로 집계 | 거부 | 공통 저자·데이터·상업적 이해관계로 correlated error 가능 |
| DDC failure에서 자동 knowledge generation·graduation | 거부 | failure 원인이 지식 부족이라는 보장이 없고 convergence는 preprint도 미검증으로 명시 |
| 성공률·click·user signal 기반 utility auto-ranking | 거부 | noisy label, credit assignment, feedback gaming, source authority와 relevance 혼동 |
| feedback으로 C-level/source level 자동 변경 | 거부 | 결과 유용성은 claim 사실성·independence·provenance의 대체물이 아님 |
| stale flag 기반 자동 invalidation·삭제 | 거부 | 오래됨은 거짓과 다르며 역사적·범위 제한 claim일 수 있음 |
| learned memory discard를 raw/Wiki 삭제 권한으로 사용 | 거부 | 논문 benchmark 범위를 넘어 provenance·retention을 훼손 |
| “log alone is the agent”를 문자 그대로 채택 | 거부 | 외부 파일, service state, credential, side effect가 log 밖에 존재 |
| graph DB를 기본 truth layer로 전환 | 유보 | entity/edge extraction 오류와 rationale fidelity가 미검증; 현재 deterministic ledger가 더 감사 가능 |
...
1. **Failure-driven curation 비교 평가**
   ...
   - 비교: top-down docs, naive append, DDC-like curation.
   - 종료: blind rubric, 동일 task set, quality/token/human-minute, counter-evidence recall 공개.

2. **Replay와 frozen regression**
   - 질문: 실제 failure를 재현하고 최소 harness change가 기존 task를 잊지 않았음을 보일 수 있는가?
   - 종료: environment digest, model/tool version, deterministic fixture, holdout contamination audit, rollback rehearsal.

3. **Feedback selection bias**
...
4. **Staleness 의미 검증**
   - 질문: `freshness=fast|normal|slow` 경고가 실제 outdated claim을 얼마나 찾고 역사적 사실을 얼마나 오탐하는가?
   - 종료: adjudicated corpus와 precision/recall. 이 전까지 staleness는 warning only다.

5. **Human memory control**
