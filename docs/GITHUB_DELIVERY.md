...
권위 문서 계약은 [Living Wiki GitHub PR 전달과 위험 기반 자동 병합](../wiki/specs/github-delivery.md)의 `SPEC-GH-DELIVERY-001` 버전 `1.2.0`이다. 연결 구현 RFC는 `RFC-03F4FE85BB44`와 전환 브리지 `RFC-7A0959853525`다. 실제 active 정책과 전환 상태의 권위는 기준 commit의 `config/github-delivery.json`이다. 브리지 단계는 `v1.1.0 + armed`, 활성화 뒤 단계는 `v1.2.0 + consumed`이며 두 값은 설정 전용 사람 검토 PR 하나에서 함께 바뀐다. 이번 전환은 기존 exact-repository 승인 범위를 넓히지 않는다.
...
- 정책 전환 근거: `COL-B80046FC1C56`, `RFC-7A0959853525`
...
구현 범위는 exact-repository 정책, 비밀 안전 loader, 결정론적 위험 분류, PR 본문과 영수증, 멱등성·SHA drift·check 판정 코어, fake transport 회귀검사와 비밀 없는 PR 품질 workflow다. 첫 사람 검토 통합 [PR #2](https://github.com/winehouse8/auto_wiki/pull/2)와 prospective gate·병합 영수증 보강 [PR #3](https://github.com/winehouse8/auto_wiki/pull/3)은 병합됐다. 이후 순서는 전환 브리지 사람 검토 병합 → 설정 전용 활성화 사람 검토 병합 → 별도 무해한 canary에서 정확한 필수 check `전체 저장소 품질 게이트`와 squash auto-merge 검증이다. 어느 단계인지의 권위는 추정 문구가 아니라 설정의 active/state, begin·delivery 영수증과 병합 SHA다. 이 순서가 끝나도 `production_certified=false`다.

## 정책 버전 전환 UX

일반 실행은 target 옵션 없이 기준 설정의 active 정책을 따른다. `v1.1.0 + armed` 브리지가 사람에 의해 병합된 뒤에만 다음 명령으로 exact one-hop 전환을 시작한다.

```bash
python3 tools/github_delivery.py begin \
  --target-policy-version SPEC-GH-DELIVERY-001/v1.2.0 \
  --invocation 수동 \
  --work-type "하네스 개선" \
  --wiki-mode wiki-first \
  --now <ISO-8601>
```

이 명령은 최신 base Git 객체의 설정 bytes와 `armed` edge를 branch 생성 전에 검증하고 from·to·base SHA·설정 SHA-256·승인·RFC proof를 영수증에 고정한다. 설정 digest는 text 줄바꿈 정규화가 아닌 Git blob 전체 bytes로 계산하고 local `refs/replace`를 무시한다. 활성화 branch에서는 `config/github-delivery.json`의 `policy_version: v1.1.0 → v1.2.0`과 같은 edge의 `state: armed → consumed` 두 값만 바꾼다. 설정 비교는 중복 key와 JSON 타입 변조도 거부하므로 `true`를 Python에서 같게 비교되는 `1`로 바꿀 수 없다. `publish`는 실제 Git diff와 base blob을 다시 검증한 뒤 draft 사람 검토 PR까지만 만든다. Agent는 이 PR을 ready·approve·merge하거나 auto-merge하지 않는다.

병합 뒤 일반 `begin`은 target 옵션 없이 v1.2로 동작한다. same-version target, 소진된 edge, 과거 v1.1 전환·일반 영수증, head에만 추가한 edge와 추가 파일 변경은 품질 gate·token·stage·transport 전에 차단한다.
...
사람이 이 PR을 병합한 뒤 원격 base가 이동하면, 자동화는 run·actor·branch·base/head/tree·멱등성 키·gate digest·PR URL과 전환 proof가 모두 일치하는 기존 review 영수증에 한해서만 그 PR 상태를 읽기 전용으로 재조회한다. 이 재조정은 실제 merge SHA를 기록하지만 새 branch·PR·label·auto-merge·approve·ready·merge 호출은 0회이며 `merge_requested=false`를 보존한다.
...
origin의 fetch URL과 push URL은 각각 정확히 하나이며 모두 승인된 `winehouse8/auto_wiki`인지 `begin`과 `publish`에서 다시 확인한다. 기준선 fetch와 인증 push는 별칭 `origin` 대신 승인된 HTTPS URL을 직접 사용한다. begin 이후 URL 변경이나 추가 push URL은 gate·token·stage·transport 전에 차단하고, 모든 Git 판단은 `refs/replace`를 무시한다.
...
각 실행은 승인된 exact HTTPS URL에서 정확한 `main` refspec으로 갱신한 `origin/main`과 일치하는 깨끗한 전용 작업 사본에서 run ID와 base SHA를 고정하고 `wiki-auto/<run-id>` 소유 브랜치를 사용한다. 첫 통합 PR을 포함한 모든 실행은 `origin/main` 기준의 별도 clean 작업 사본에서 `begin` 영수증을 먼저 만든 뒤 승인된 patch만 적용한다. 기존 dirty 기본 작업 사본을 직접 stage하거나 시작 영수증 이전 파일을 manifest로 추측하지 않는다. 모든 begin 영수증은 publish 전에 최신 기준선과 다시 대조하며, base가 이동한 오래된 일반 영수증은 품질 gate와 token read 전에 폐기한다. 사람 검토 PR의 결속된 읽기 전용 재조정만 위에서 정한 예외다.
...
일반 실행의 멱등성 키는 `policy version + run ID + base SHA + tree SHA`다. 전환 실행은 여기에 target 정책과 base 정책 설정 SHA-256을 추가해 다른 edge나 설정에서 키를 재사용하지 못하게 한다. timeout 뒤에는 생성·병합을 곧바로 반복하지 않고 원격 브랜치, PR marker와 merge 상태를 조회한다. 같은 키는 branch·PR·auto-merge 요청을 각각 최대 한 번만 만든다. force push, admin bypass, force merge와 다른 실행의 브랜치 재사용은 금지한다.
