# Memory feedback and non-destructive lifecycle
...
- stable report ID와 full SHA-256
...
```text
active      → deprecated | superseded | invalidated | archived
deprecated  → active | superseded | invalidated | archived
invalidated → active | superseded | archived
superseded  → archived
archived    → terminal
```
...
replaced_by             # superseded에서만 새로 설정
lifecycle_history[]     # 전체 transition envelope
```
