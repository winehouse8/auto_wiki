---
type: "Reference"
title: "LLM Wiki"
description: "출처 SRC-CFB88DDE3FF1의 범위 한정 평가와 보존 정보. 현재 레벨은 S2."
resource: "https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f"
tags: ["source", "article", "S2", "active"]
timestamp: "2026-07-11T14:28:47+00:00"
assessed_at: "2026-07-11T14:28:47+00:00"
generated: true
lifecycle_status: "active"
project_ids: ["PRJ-WIKI-HARNESS"]
retrieved_at: "2026-07-11T14:28:47+00:00"
source_id: "SRC-CFB88DDE3FF1"
source_level: "S2"
---
<!-- state/sources.json에서 자동 생성함. -->
# LLM Wiki

## 출처 식별 정보

| 항목 | 값 |
|---|---|
| 정규 출처 ID | `SRC-CFB88DDE3FF1` |
| 유형 | `article` |
| 저자 | Andrej Karpathy |
| 발행자 | GitHub Gist |
| 출판 상태 | practitioner idea file |
| 게시 시각 | 2026-04-04 |
| 검색 시각 | 2026-07-11T14:28:47+00:00 |
| 독립성 그룹 | `karpathy-llm-wiki` |
| 라이선스 | - |
| 생명주기 | **active** |
| 생명주기 사유 | - |
| 대체 출처 | - |

## 범위 한정 평가

- 레벨: **S2**
- 기록된 평가 근거: 저자의 설계 의도를 확인하는 1차 자료지만 동료심사·벤치마크·참조 구현이 없는 아이디어 파일이다.
- 품질 표지: `primary-author-account`
- 이해관계 충돌: 기록 없음

## 보존 원문

- bundle 밖 경로: `메타데이터 전용`
- SHA-256: `없음`
- 미디어 유형: `없음`

## 이 출처를 사용하는 주장

- [CLM-6DDD53332A40](../claims/clm-6ddd53332a40.md)
- [CLM-F6367BFF8F35](../claims/clm-f6367bff8f35.md)

## 관련 연구 프로젝트

- 연구 프로젝트: [PRJ-WIKI-HARNESS](../projects/prj-wiki-harness.md)

# 인용

[1] [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)
