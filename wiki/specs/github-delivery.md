...
버전 `1.2.0`은 첫 통합 PR 병합 뒤 발견한 prospective gate의 자기 변경과 사람·비동기 자동 병합 영수증 재조정 결함을 좁게 보강한다. [PR #3](https://github.com/winehouse8/auto_wiki/pull/3)은 이 보강을 사람 검토로 병합했지만 운영 코드·설정은 전환을 위해 `SPEC-GH-DELIVERY-001/v1.1.0` 식별자를 유지한다. `RFC-7A0959853525`의 검토된 단일 단계 전환 브리지가 먼저 병합된 뒤에만 정확한 `v1.1.0 → v1.2.0` 전환 영수증으로 v1.2를 활성화한다. 기존 승인 저장소·브랜치·행위자·병합 방식과 RFC 승인 범위를 넓히지 않으며, production 인증도 추가하지 않는다.
...
원격 identity는 fetch URL과 push URL을 각각 단 하나만 허용하며 둘 다 정확한 `winehouse8/auto_wiki`여야 한다. `begin`과 `publish`는 이를 다시 확인하고, 기준선 fetch와 인증 push는 검증 뒤에도 별칭 `origin`을 실행 대상으로 쓰지 않고 승인된 HTTPS URL과 정확한 `main` refspec을 직접 사용한다. begin 영수증 이후 URL이 바뀌거나 추가 push URL이 생기면 token read와 외부 쓰기 전에 차단한다. 모든 Git 객체 판정과 push 자식 프로세스는 local `refs/replace/**`를 무시해 같은 SHA 문자열 아래 다른 commit·tree를 대입할 수 없게 한다.
...
1. `begin`은 clean worktree, 단일 exact fetch·push URL, 최신 기준 SHA를 확인하고 `wiki-auto/<run-id>` 브랜치와 Git 내부 실행 영수증을 만든다.
...
사람 검토 PR이 외부에서 병합된 뒤 재조정할 때는 실제 merge SHA와 원격 상태를 기록하되 자동화가 auto-merge를 요청했다고 소급해 기록하지 않는다. base가 이동한 뒤 허용하는 예외는 run·actor·branch·base/head/tree·멱등성 키·gate digest·PR URL과 전환 proof가 모두 일치하는 기존 draft 사람 검토 영수증의 읽기 전용 재조회뿐이다. 이 경로는 새 branch·PR·auto-merge·ready·approve·merge 호출을 0회로 유지하며, 기존 PR이 없거나 닫혔거나 다른 위험 경로이면 차단한다. 원격 `autoMergeRequest`나 기존 자동 요청 영수증이 없는 사람 검토 경로는 `merge_requested=false`를 보존한다. 반대로 같은 멱등성 키와 PR의 이전 safe 영수증이 auto-merge 요청을 증명하면 GitHub가 병합 뒤 활성 요청 필드를 비워도 `merge_requested=true`를 보존한다. classic PAT가 사람과 Agent 행위자를 구분하지 못하므로 병합 주체를 근거 없이 특정하지 않는다.
...
운영 정책 식별자를 바꾸는 실행은 일반 `begin → publish`의 exact-version 검사를 약화하지 않고, 사람이 검토해 기준 브랜치에 미리 병합한 전환 edge로만 시작한다. 첫 허용 edge는 `SPEC-GH-DELIVERY-001/v1.1.0 → SPEC-GH-DELIVERY-001/v1.2.0` 하나다. 같은 버전, 임의 후속 버전, 역방향과 두 단계 건너뛰기는 허용하지 않는다.

1. 기준 `config/github-delivery.json`은 현재 active 정책, `armed` 상태의 from·to, 정책 승인자와 RFC 참조를 명시한다. 전환 edge와 두 정책을 이해하는 version-neutral verifier를 추가하는 브리지 자체는 기존 v1.1 정상 `begin → publish`로 만들며 active 정책을 바꾸지 않는 draft 사람 검토 PR을 통해 먼저 병합한다.
2. `begin --target-policy-version <to>`는 clean 최신 `main`에서 실행 중 코드와 설정의 active 버전이 from과 같은지, target이 기준 설정의 유일한 `armed` edge와 같은지 검증한다. branch를 만들기 전에 from·to·base SHA·기준 설정 Git blob 전체 bytes의 SHA-256·승인 참조·RFC proof를 메모리에 고정한다. text mode 줄바꿈 정규화로 CRLF를 LF로 바꾼 digest는 허용하지 않으며, branch 생성이 성공한 뒤 같은 proof를 begin 영수증에 원자적으로 기록한다.
3. 전환 `publish`는 begin에서 고정한 base SHA의 설정을 replacement ref가 적용되지 않은 Git object에서 다시 읽고 digest와 `armed` edge를 확인한다. 현재 head만 바꾼 설정이나 context가 새 target을 승인할 수 없다. base drift, receipt 변조, 승인·RFC 누락과 설정 digest 불일치는 token read, gate, stage, commit과 transport 전에 차단한다.
4. 전환 head는 브리지에서 검토된 verifier와 실행 경로의 bytes를 바꾸지 않고 현재 설정의 active 버전만 to로 전환하며 같은 edge를 `consumed`로 바꿔야 한다. JSON의 key뿐 아니라 값의 타입도 보존하므로 `true`와 `1`처럼 Python 비교에서 같게 보이는 변조도 추가 변경이다. base 설정에서 허용한 이 두 값 외의 추가 설정 변경, verifier·workflow·스크립트 변경과 부분 전환은 차단한다.
5. 전환 manifest는 정확한 설정 파일 하나이며 항상 `사람 검토 필요`여야 한다. draft PR만 만들고 auto-merge·approve·ready·merge 호출은 0회다. 전환 멱등성 키는 from·to와 기준 설정 digest를 모두 포함하되 target이 없는 기존 실행의 키 계산은 바꾸지 않는다.
6. 전환 PR이 사람에 의해 병합된 뒤에는 일반 v1.2 `begin`이 새 기준선에서 성공해야 한다. 소진된 edge와 과거 v1.1 begin 영수증은 새 전환이나 게시 권한으로 재사용할 수 없고, 일반 과거 영수증도 이동한 base를 gate·token·stage·transport보다 먼저 확인해 차단한다. 단, 위에서 고정한 기존 사람 검토 PR의 읽기 전용 병합 재조정은 허용한다.

이 브리지는 정확한 한 번의 정책 식별자 이동만 허용하며 저장소·권한·위험 분류·병합 방식이나 승인 범위를 넓히지 않는다. 새 정책 버전마다 별도의 기준 edge와 사람 검토가 필요하다.
...
- `AC-GH-030`: 이미 병합된 사람 검토 PR은 prior receipt의 run·actor·SHA·tree·멱등성 키·gate·PR URL이 모두 일치할 때만 base drift 뒤 읽기 전용 재조정한다. 실제 merge SHA를 기록하되 새 branch·PR·auto-merge·ready·approve·merge 호출은 0회이고 `merge_requested=false`를 유지해 자동 병합 요청을 소급 주장하지 않는다.
- `AC-GH-031`: `begin --target-policy-version`은 clean 최신 기준선, 단일 exact fetch·push URL, 실행 코드·active 설정·사람이 검토한 유일한 `armed` from→to edge가 모두 일치할 때만 전환을 시작한다. from·to·base SHA·줄바꿈 정규화 없는 기준 설정 Git blob bytes의 SHA-256·승인·RFC proof는 branch 생성 전에 검증·고정하고, branch 성공 뒤 같은 proof를 영수증에 원자 기록한다.
- `AC-GH-032`: 전환 `publish`는 replacement ref를 무시하고 영수증과 base Git object의 설정 digest·edge, bridge verifier의 target 지원, head active 설정의 target 수렴과 `consumed` 상태, 비전환 JSON 값의 타입까지 token read·gate·stage·transport 전에 검증하며 same-version, unsupported, head-only edge, receipt 변조, URL drift, base drift와 부분 전환을 거부한다.
- `AC-GH-033`: 전환 diff는 항상 draft 사람 검토 PR이며 auto-merge·approve·ready·merge 호출이 0회다. 전환 멱등성 키에는 from과 to가 모두 들어가고 일반 실행의 기존 키는 보존되며, 병합 뒤 소진된 edge와 과거 from 영수증은 재사용되지 않는다.
- `AC-GH-034`: 전환 브리지 PR은 normal v1.1 `begin → publish`로 생성되고 active 정책을 v1.1로 유지하며, version-neutral verifier와 정확한 `armed` edge를 draft 사람 검토로만 전달한다.
- `AC-GH-035`: 활성화 PR은 base verifier·workflow·스크립트를 바꾸지 않고 `config/github-delivery.json`의 active 버전과 같은 edge의 상태만 각각 target·`consumed`로 바꾼다. 병합 뒤 normal v1.2 `begin`은 성공하고 과거 v1.1 전환·일반 영수증과 소진 edge는 gate·token read 전에 거부된다.
