...
빈 저장소에 기준 ref를 만드는 일회성 예외는 깨끗한 로컬 커밋 `5f1d7f0`만 `main`에 게시하면서 이미 소진했습니다. 공개 감사 영수증은 [GitHub 이슈 #1](https://github.com/winehouse8/auto_wiki/issues/1)입니다. 이후 자동 작업의 `main` 직접 push는 허용되지 않습니다. 첫 GitHub 통합 변경 [PR #2](https://github.com/winehouse8/auto_wiki/pull/2)와 v1.2 제어면 보강 [PR #3](https://github.com/winehouse8/auto_wiki/pull/3)은 사람이 검토해 병합했습니다. 이후 순서는 exact one-hop 전환 브리지 사람 검토 → 설정 전용 `v1.2 + consumed` 활성화 사람 검토 → 별도의 무해한 원격 check·자동 병합 canary입니다. 현재 단계의 권위는 `config/github-delivery.json`의 active/state, begin·delivery 영수증과 PR merge SHA입니다.
...
현재 v4.3은 첫 사람 검토 통합 PR, v1.2 제어면 보강, 일회성 seed와 branch protection까지 적용했습니다. 설정이 `v1.1 + armed`이면 활성화 전이고, `v1.2 + consumed`이면 활성화 PR 병합 뒤 canary 대기 상태입니다. 무해한 원격 canary와 Codex 예약 작업 화면의 활성·다음 실행 시각·최초 실행 영수증까지 확인하기 전에는 자동 전달이나 예약 연구가 운영 환경에서 검증됐다고 말하지 않으며 `production_certified=false`를 유지합니다. 상세 운영·복구 절차는 [GitHub PR 전달 운영 안내](docs/GITHUB_DELIVERY.md), 권위 계약은 [SPEC-GH-DELIVERY-001](wiki/specs/github-delivery.md)을 따릅니다.
...
GitHub 전달은 단일 exact fetch·push URL, replacement-ref 무시, Git blob bytes와 JSON 타입 보존, 오래된 영수증의 자격증명 전 차단, 사람 병합 review PR의 읽기 전용 재조정을 강제합니다. 상세 운영·복구 절차는 [GitHub PR 전달 운영 안내](docs/GITHUB_DELIVERY.md)를 따릅니다.
