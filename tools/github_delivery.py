...
정책 계약: SPEC-GH-DELIVERY-001/v1.1.0
"""
...
def _command(
    ...
    *,
    ...
    try:
        result = runner.run(
            tuple(argv), cwd=cwd, env=env, input_text=input_text
        )
    except Exception:
...
def _validate_policy_context(context: Mapping[str, Any]) -> None:
    ...
    if (
        not isinstance(approval_refs, Sequence)
        or isinstance(approval_refs, (str, bytes))
    ...
    if (
        not isinstance(rfc_ids, Sequence)
        or isinstance(rfc_ids, (str, bytes))
...
def idempotency_key(
    *, policy_version: str, run_id: str, base_sha: str, tree_sha: str
) -> str:
    """정책 계약의 네 구성요소를 포함하는 안정적인 SHA-256 key를 만든다."""

    payload = json.dumps(
        [policy_version, run_id, base_sha, tree_sha],
        ensure_ascii=False,
        separators=(",", ":"),
    ).encode("utf-8")
...
def _deliver_impl(
    ...
    _validate_policy_context(request)
...
def begin_run(
    ...
    *,
    ...
    _validate_policy_context(context)
    root = Path(repo_root).resolve()
    ...
    origin = _command(
        ...
        cwd=root,
        purpose="origin identity 확인",
    )
    if _origin_repository(_output(origin)) != EXPECTED_REPOSITORY:
        raise DeliveryBlocked("origin이 승인된 정확한 GitHub 저장소가 아님")
    ...
    _command(
        runner,
        ("git", "fetch", "--no-tags", "--prune", "origin", EXPECTED_BASE_BRANCH),
        cwd=root,
...
def _git_base_text(root: Path, runner: Any, *, base_sha: str, path: str) -> bytes | None:
    ...
    if getattr(result, "returncode", None) != 0:
        return None
    return _output(result).encode("utf-8")
    ...
    *,
    ...
    for index, (path, observed) in enumerate(sorted(discovered.items())):
        ...
        if path == "state/events.jsonl" and status_value == "modified":
            base_data = _git_base_text(root, runner, base_sha=base_sha, path=path)
...
def _publish_run_unlocked(
    ...
    *,
    ...
    if not manifest:
        ...
        return deliver(context, transport=transport, token_loader=token_loader)
    ...
    try:
        if args.command == "begin":
            ...
            context = {
                ...
                "approval_refs": [EXPECTED_APPROVAL_ID],
            }
            receipt = begin_run(
                context,
                repo_root=root,
