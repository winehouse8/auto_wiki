...
7. 외부 자료는 canonical source writer로 직행하지 않는다.
...
14. 작업이 bounded `RUN/ACT` 계획에서 시작됐다면 실제 사용량과 evidence reference를 `run-action-report`에 기록한다. 이 receipt는 `unverified_report`이므로 사실성 검토를 대신하지 않는다.
